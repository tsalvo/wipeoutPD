...
void ships_load(PlaydateAPI *pd) {
	pd->system->logToConsole("ships_load");
	texture_list_t ship_textures = texture_list_empty();// image_get_compressed_textures("wipeout/common/allsh.cmp");
...
void ship_init(ship_t *self, section_t *section, int pilot, int inv_start_rank) {
	...
	self->start_accelerate_timer = p * (spread_base + (p * spread_factor)) * (1.0F/30.0F);
	...
	vec3_t face_point = vec3_mulf(vec3_add(face->tris[0].vertices[0], face->tris[0].vertices[2]), 0.5);
	...
	for (int i = 0; i < self->model->primitives_len; i++) {
		...
		case PRM_TYPE_F3 :
			if (flags_is(prm.f3->flag, PRM_SHIP_ENGINE)) {
				// printf("F3 ::SE marked polys should be ft3's");
			}
			prm.f3 += 1;
			break;
		...
		case PRM_TYPE_F4 :
			if (flags_is(prm.f4->flag, PRM_SHIP_ENGINE)) {
				// printf("F4 ::SE marked polys should be ft3's");
			}
			prm.f4 += 1;
			break;
		...
		case PRM_TYPE_FT4 :
			if (flags_is(prm.ft4->flag, PRM_SHIP_ENGINE)) {
				// printf("FT4 ::SE marked polys should be ft3's");
			}
			prm.ft4 += 1;
			break;
		...
		case PRM_TYPE_G3 :
			if (flags_is(prm.g3->flag, PRM_SHIP_ENGINE)) {
				// printf("G3 ::SE marked polys should be ft3's");
			}
			prm.g3 += 1;
			break;
		...
		case PRM_TYPE_G4 :
			if (flags_is(prm.g4->flag, PRM_SHIP_ENGINE)) {
				// printf("G4 ::SE marked polys should be ft3's");
			}
			prm.g4 += 1;
			break;
		...
		case PRM_TYPE_GT4 :
			if (flags_is(prm.gt4->flag, PRM_SHIP_ENGINE)) {
				// printf("GT4 ::SE marked polys should be ft3's");
			}
			prm.gt4 += 1;
			break;

		default :
			// printf("cone.c::InitCone:Bad primitive type %x\n", prm.f3->type);
			break;
		}
	}
...
void ship_update(ship_t *self, PlaydateAPI *pd) {
	...
	if (self->pilot == g.pilot) {
		...
		exhaust_len = self->thrust_mag * 0.0625F;
		exhaust_len += self->speed * 0.00390625F;
	}
	else {
...
static bool vec3_is_on_face(vec3_t pos, track_face_t *face, float alpha) {
	...
	return (angle > M_PIF * 2.0F - 0.01F);
}

void ship_resolve_wing_collision(ship_t *self, track_face_t *face, float direction) {
	vec3_t collision_vector = vec3_sub(self->section->center, face->tris[0].vertices[2]);
	...
	float magnitude = (fabsf(angle) * self->speed) * M_PIF / (4096.0F * 16.0F); // (6 velocity shift, 12 angle shift?)
...
void ship_resolve_nose_collision(ship_t *self, track_face_t *face, float direction) {
	...
	float magnitude = ((self->speed * 0.0625F) + 400.0F) * M_PIF / (4096.0F * 64.0F);
	if (direction > 0) {
...
void ship_collide_with_track(ship_t *self, track_face_t *face) {
	...
	vec3_t to_face_vector = vec3_sub(face->tris[0].vertices[0], face->tris[0].vertices[1]);
