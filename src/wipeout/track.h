...
vec3_t *track_load_vertices(char *file, PlaydateAPI *pd);
void track_load_faces(char *file, vec3_t *vertices, PlaydateAPI *pd);
void track_load_sections(char *file, PlaydateAPI *pd);
...
struct camera_t;
void track_draw(struct camera_t *camera, PlaydateAPI *pd);
