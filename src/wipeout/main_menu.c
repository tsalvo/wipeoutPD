...
static void page_options_init(menu_t *menu) {
	...
	menu_page_add_button(page, 0, "VIDEO", button_video);
	menu_page_add_button(page, 1, "AUDIO", button_audio);
...
// 	await_input_deadline = pd->system->getElapsedTime() + 3;
// 
...
static void page_options_control_draw(menu_t *menu, int data, PlaydateAPI *pd) {
// 	menu_page_t *page = &menu->pages[menu->index];
// 
...
// 	ui_draw_text("KEYBOARD", ui_scaled_pos(page->items_anchor, left_head_pos, pd), UI_SIZE_8, UI_COLOR_DEFAULT, pd);
// 
// 	vec2i_t right_head_pos = vec2i(right - ui_text_width("JOYSTICK", UI_SIZE_8), line_y);
// 	ui_draw_text("JOYSTICK", ui_scaled_pos(page->items_anchor, right_head_pos, pd), UI_SIZE_8, UI_COLOR_DEFAULT, pd);
// 	line_y += 20;
// 
...
static void page_race_class_draw(menu_t *menu, int data, PlaydateAPI *pd) {
	menu_page_t *page = &menu->pages[menu->index];
	flags_add(page->layout_flags, MENU_FIXED);
	page->title_pos = vec2i(0, 30);
	page->title_anchor = UI_POS_TOP | UI_POS_CENTER;
...
static void page_circut_draw(menu_t *menu, int data, PlaydateAPI *pd) {
...
void main_menu_init(PlaydateAPI *pd) {
	...
	objects_unpack(models.race_classes, objects_load("wipeout/common/leeg.prm", texture_list_empty(), pd));
	objects_unpack(models.teams, objects_load("wipeout/common/teams.prm", texture_list_empty(), pd));
	objects_unpack(models.pilots, objects_load("wipeout/common/pilot.prm", texture_list_empty(), pd));
	objects_unpack(models.options, objects_load("wipeout/common/alopt.prm", texture_list_empty(), pd));
	objects_unpack(models.rescue, objects_load("wipeout/common/rescu.prm", texture_list_empty(), pd));
	objects_unpack(models.controller, objects_load("wipeout/common/pad1.prm", texture_list_empty(), pd));
	objects_unpack(models.misc, objects_load("wipeout/common/msdos.prm", texture_list_empty(), pd));
