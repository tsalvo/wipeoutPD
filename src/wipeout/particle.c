...
void particles_load(PlaydateAPI *pd) {
	// // printf("mem bump particle %lu bytes\n", sizeof(particle_t) * PARTICLES_MAX);
...
void particles_draw(PlaydateAPI *pd) {
...
// 	for (int i = 0; i < particles_active; i++) {
// 		particle_t *p = &particles[i];
// 		render_push_sprite(p->position, p->size, p->color, 0);
// 	}
// 
