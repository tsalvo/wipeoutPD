...
vec2i_t ui_scaled_screen(PlaydateAPI *pd) {
	return vec2i_mulf(vec2i(pd->display->getWidth(), pd->display->getHeight()), ui_scale);
}

vec2i_t ui_scaled_pos(ui_pos_t anchor, vec2i_t offset, PlaydateAPI *pd) {
	vec2i_t pos;
	vec2i_t screen_size = vec2i(pd->display->getWidth(), pd->display->getHeight());
